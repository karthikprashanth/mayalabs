...
<div class="forums-wrapper">
	<table class="forums">
		<?php if (sizeof($this->_tpldata['user_row'])) {  ?>
			...
			<thead>
				<tr>
					<th class="username"><a href="<?php echo (isset($this->_rootref['U_SORT_USERNAME'])) ? $this->_rootref['U_SORT_USERNAME'] : ''; ?>"><?php echo ((isset($this->_rootref['L_USERNAME'])) ? $this->_rootref['L_USERNAME'] : ((isset($user->lang['USERNAME'])) ? $user->lang['USERNAME'] : '{ USERNAME }')); ?></a></th>
					<th class="ip"><?php echo ((isset($this->_rootref['L_IP'])) ? $this->_rootref['L_IP'] : ((isset($user->lang['IP'])) ? $user->lang['IP'] : '{ IP }')); ?></th>
					<th class="forum-location"><a href="<?php echo (isset($this->_rootref['U_SORT_LOCATION'])) ? $this->_rootref['U_SORT_LOCATION'] : ''; ?>"><?php echo ((isset($this->_rootref['L_FORUM_LOCATION'])) ? $this->_rootref['L_FORUM_LOCATION'] : ((isset($user->lang['FORUM_LOCATION'])) ? $user->lang['FORUM_LOCATION'] : '{ FORUM_LOCATION }')); ?></a></th>
			...
			<?php $_user_row_count = (isset($this->_tpldata['user_row'])) ? sizeof($this->_tpldata['user_row']) : 0;if ($_user_row_count) {for ($_user_row_i = 0; $_user_row_i < $_user_row_count; ++$_user_row_i){$_user_row_val = &$this->_tpldata['user_row'][$_user_row_i]; ?>
				...
				<tr class="<?php if (($_user_row_val['S_ROW_COUNT'] & 1)  ) {  ?>bg2<?php } else { ?>bg1<?php } ?>">
					<td>
						<p><?php echo $_user_row_val['USERNAME_FULL']; ?></p>
						<?php if ($_user_row_val['USER_BROWSER']) {  ?><p><?php echo $_user_row_val['USER_BROWSER']; ?></p><?php } ?>

					</td>
					<td class="bg2">
						<?php if ($_user_row_val['USER_IP']) {  ?>

							<p><a href="<?php echo $_user_row_val['U_USER_IP']; ?>"><?php echo $_user_row_val['USER_IP']; ?></a></p>
							<p><a href="<?php echo $_user_row_val['U_WHOIS']; ?>" onclick="popup(this.href, 750, 500); return false;"><?php echo ((isset($this->_rootref['L_WHOIS'])) ? $this->_rootref['L_WHOIS'] : ((isset($user->lang['WHOIS'])) ? $user->lang['WHOIS'] : '{ WHOIS }')); ?></a></p>
						<?php } else { ?>
					...
					<td><p><a href="<?php echo $_user_row_val['U_FORUM_LOCATION']; ?>"><?php echo $_user_row_val['FORUM_LOCATION']; ?></a></p></td>
					<td class="bg2"><p><?php echo $_user_row_val['LASTUPDATE']; ?></p></td>
				</tr>
			<?php }} } else { ?>
